...
export default {
    ...
    setup(props, { emit }) {
        ...
        const isImage = (file) => {
            ...
            return imageTypes.includes(file.content_type || file.type);
        };
        ...
        const handleSubmit = () => {
            ...
            props.fields.forEach(field => {
                if (field.type === 'file') {
                    if (selectedFiles[field.key]?.length > 0) {
                        selectedFiles[field.key].forEach(file => {
                            formData.append(field.key, file);
                        });
                    } else if (initialFiles[field.key]?.length > 0) {
                        initialFiles[field.key].forEach(file => {
                            formData.append(`${field.key}`, JSON.stringify(file));
                        });
                    } else {
                        formData.append(field.key, '');
                    }
                } else {
                    formData.append(field.key, getFieldModelValue(field.key) || '');
                }
            });
            ...
            Object.keys(translatedData).forEach(locale => {
                props.fields.forEach(field => {
                    if (field.translatable) {
                        const translationKey = `Translatable[${locale}][${field.key}]`;
                        formData.append(translationKey, translatedData[locale][field.key] || '');
                    }
                });
            });
